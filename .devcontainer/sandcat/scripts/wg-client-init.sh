...
# ── Override DNS ──────────────────────────────────────────────────────────
# Docker's embedded DNS at 127.0.0.11 resolves container names on the
# Docker network (e.g. mysql, redis). Keep it as the first nameserver so
# sibling services added to compose-all.yml are reachable by name.
# External queries that Docker DNS cannot answer fall through to
# 1.1.1.1 / 8.8.8.8, which are routed through wg0 and intercepted by
# mitmproxy.

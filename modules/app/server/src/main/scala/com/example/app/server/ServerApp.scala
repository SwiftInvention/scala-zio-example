...
object ServerApp extends ZIOAppDefault {
  ...
  /** Install the configured logger before any layer in the main `run` effect builds, so the very first log line uses
  ...
  def run: ZIO[Any, Throwable, Unit] = {
    ...
    // Top-level cause log — last-resort visibility before the runtime swallows.
    serve

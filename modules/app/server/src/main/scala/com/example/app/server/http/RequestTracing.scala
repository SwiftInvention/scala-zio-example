...
/** Server-span middleware: opens one span per HTTP request, named `<METHOD> <path>`, kind `SERVER`. The handler runs
  ...
  *
  * The middleware adds `Tracing` to the env requirement of the routes it's applied to. `ServerEnv` wires `Tracing` so
  * the dependency is satisfied at composition time.
  *
